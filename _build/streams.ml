...
let rec zipt (f: 'a -> 'b -> 'c) (t1: 'a tree) (t2: 'b tree) : 'c tree =
  let Stem(a, lt1, rt1) = t1 () in
  let Stem(b, lt2, rt2) = t2 () in
  fun () -> Stem(f a b, zipt f lt1 lt2, zipt f rt1 rt2)
;;
...
let rec treenats () =
  fun () -> Stem(1, mapt (( * ) 2) (treenats ()), mapt (fun x -> (x * 2) + 1) (treenats ()))
;;
...
let tail (s:'a stream) : 'a stream lazy_t =
   let Cons(_, t) = s in t
;;

let rec map (f:'a -> 'b) (s:'a stream) : 'b stream =
  Cons(f (head s), lazy(map f (Lazy.force (tail s))))
;;
...
let rec extract (value : 'a option) : 'a =
    match value with
    |None -> failwith "value should exist"
    |Some a -> a 

let rec nth_helper (n:int) (s:'a stream) : 'a list =
    if n >= 0 
    then head s :: nth_helper (n - 1) (Lazy.force (tail s))  
    else []

let nth (n:int)(s:'a stream) : 'a =
    let nth_list = nth_helper n s in
    extract (List.hd (List.rev nth_list))     
;;
...
let rec merge (s1:int stream) (s2:int stream) : int stream =
    ...
    if a = b then 
        Cons(a, lazy (merge (Lazy.force t1) (Lazy.force t2)))  
    else if a > b then 
        Cons(b, lazy (merge s1 (Lazy.force t2)))
    else
      Cons(a, lazy (merge (Lazy.force t1) s2))  
;;
...
(*
 *  Answer: If we were to merge ones and ones, we would run into an infinite loop on the tails. 
 *)
